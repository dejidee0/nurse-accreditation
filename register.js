...
function initializeForm() {
    ...
    const savedData = NNEC.getFromLocalStorage('nnec_registration_data');
    if (savedData) {
        formData = savedData;
...
function handleUserTypeChange() {
    ...
    if (selectedType) {
        ...
        // Save progress
        NNEC.saveToLocalStorage('nnec_registration_data', formData);
...
function showStep(stepNumber) {
    ...
    const currentProgressStep = document.querySelector(`[data-step="${stepNumber}"]`);
...
function validateStep2() {
    ...
    requiredFields.forEach(field => {
        ...
        // Email validation
        if (field.type === 'email' && field.value && !NNEC.validateEmail(field.value)) {
            isValid = false;
        }
        
        // Phone validation
        if (field.type === 'tel' && field.value && !NNEC.validatePhone(field.value)) {
            isValid = false;
        }
    });
...
function showFormErrors() {
    const requiredFields = personalInfoForm.querySelectorAll('input[required], select[required]');
...
function validateStep3() {
    ...
    const hasRequiredFiles = requiredFiles.every(file => 
        uploadedFiles.includes(file) || uploadedFiles.includes(file.replace('File', ''))
    );
...
function handleSubmitRegistration() {
    if (validateStep3()) {
        ...
        setTimeout(() => {
            ...
            // Clear saved data
            localStorage.removeItem('nnec_registration_data');
    ...
    modal.innerHTML = `
        ...
        <div class="modal-content">
            ...
            <div class="reference-number">
                <strong>Reference Number: NNEC${Date.now()}</strong>
            </div>
